...
> Source ID: `posthog-stamphog-source-2`  
> Original URL: <https://posthog.com/blog/10k-prs-a-month>  
> Final URL: <https://posthog.com/blog/10k-prs-a-month>  
> Title: 10,000 PRs a month is easy: How devex is evolving at PostHog - PostHog  
> Captured at: `2026-08-31T17:46:04Z`

---

![10,000 PRs a month is easy: How devex is evolving at PostHog](https://res.cloudinary.com/dmukukwp6/image/upload/f_webp,h_823,w_1568,c_fill,g_auto,q_auto/stop_being_the_code_review_bottleneck_hero_a734a37558?_a=AXAH4nI0)
...
Everybody's using agents and [writing skills](https://posthog.com/newsletter/writing-agent-skills), but our [self-driving loop](https://posthog.com/newsletter/loops) is further fueling the accelerated cadence. Over the last 4 months, we moved from around 20% of our monorepo PRs being opened by agent to 70%. [Self-driving](https://posthog.com/blog/self-driving-product) is around 4% of our PRs right now but growing fast.
...
[Depot](https://depot.dev/customers/posthog), the compute infrastructure for our CI, has handled this incredibly well – in June 2026, we used 21,643,653 minutes of CI time. That's 41 *years* of CI time in a single month.
...
Alongside all this, we're streaming [GitHub data](https://posthog.com/docs/data-warehouse/sources/github) into PostHog. This gives us engineering analytics that we can look at using [PostHog AI](https://posthog.com/ai):
...
With agents creating more and more of the PRs, [code review](https://posthog.com/newsletter/code-review-tips) becomes a bigger part of the work. Engineers spend less time on each review, so we need to find ways of making that safe while maintaining momentum.
...
This means we can have a world where humans review code less. And the code reviews they do are more effective as a result. We're continuing to work on [tools and agentic flows we can use in CI](https://posthog.com/newsletter/software-factories#what-is-a-software-factory) to make it safe to operate at this new, increased speed.
...
[Pawel](https://posthog.com/community/profiles/33209) built an early version of this with a [`qa-frontend`](https://github.com/PostHog/posthog/pull/72523) skill that checks out a PR, designs test cases, and drives the actual UI in a browser. The agent's output lands as annotated screenshots, an animated reel inline, and even an optional recorded video.
