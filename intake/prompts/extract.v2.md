...
6. Eligibility is categorical, from the contribution guide's three questions:
   which organization uses the system and for what internal work; what that
   organization built or materially adapted; what implementation or use the
   sources describe. Set `decision` to `out-of-scope` only when a rule clearly
   fails — general model-serving infrastructure with no documented agent
   workflow, or a story that only reports adoption — and to `needs-evidence`
   when a required fact stays unclear. Never score eligibility numerically.
