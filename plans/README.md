...
| [017](017-source-intake-pipeline.md) | Turn a list of links into a reviewed catalog record | P1 | L | 016, the schema simplification review | IMPLEMENTED 2026-09-22 on `claude/plan-017-source-intake-pipeline`; Phases 0–5 done (all twelve stages, queue runs, review sheets, run manifests, backfill apply, drift report, skill, operator doc; every gate command exists — `run`, `backtest --records all`, `evals --score`, `backfill`/`backfill-apply`, `drift`; 124 tests over real captures with fake services; drift schedule rejected per open decision 4); live gates — capture, backtest, judgments, acceptance, one hand drift run — blocked on API keys |
