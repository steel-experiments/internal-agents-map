...
| [017](017-source-intake-pipeline.md) | Turn a list of links into a reviewed catalog record | P1 | L | 016, the schema simplification review | IMPLEMENTED 2026-09-22 on `claude/plan-017-source-intake-pipeline`; Phases 0–5 done (all twelve stages, queue runs, additive update merge with `contradicts` proposals, cross-source number checks, drift with advisory re-judging, backfill apply, review sheets, run manifests, byte-identical warm reruns, in-run private-data guard, skill, operator doc; every gate command exists — `run`, `backtest --records all`, `evals --score`, `backfill`/`backfill-apply`, `drift`; 186 tests over real captures with fake services; drift schedule rejected per open decision 4); live gates — capture, backtest, judgments, acceptance, one hand drift run — blocked on API keys |
